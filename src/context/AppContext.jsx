...
import { apiFetch, setTokens, getAccessToken, getRefreshToken, clearTokens, downloadAuthenticatedFile, requestEmailOtp as requestEmailOtpApi, verifyEmailOtp as verifyEmailOtpApi, uploadAvatarApi, deleteAvatarApi } from '../api/client';
import { INDEXPILOT_SUBSCRIPTION_PLANS, INDEXPILOT_FREE_TIER } from '../data/mockData';

const AppContext = createContext(null);

const formatAvatarUrl = (url, token) => {
  if (!url) return null;
  const activeToken = token || getAccessToken();
  if (!activeToken) return url;
  if (url.startsWith('/api/') || url.includes('/profile/avatar') || url.includes('/profile/photo')) {
    const cleanUrl = url.split('?')[0];
    return `${cleanUrl}?token=${encodeURIComponent(activeToken)}`;
  }
  return url;
};
...
export const AppProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const restoreSession = async () => {
      ...
      if (!activeToken && !getRefreshToken()) {
  ...
  const login = async (identifier, password, rememberMe = false, product = null) => {
    try {
      const res = await apiFetch('/auth/login', {
        method: 'POST',
        body: { identifier, password, rememberMe, product },
        auth: false,
      });
      ...
      return { success: false, error: res.data?.error || 'Invalid email/mobile or password.', code: res.data?.code };
    } catch (err) {
  ...
  const requestEmailOtp = async (data) => {
    ...
    return res.ok
      ...
      : { success: false, error: res.data?.error || 'Could not send verification code.', code: res.data?.code, retryAfterSeconds: res.data?.retryAfterSeconds || 0 };
  };

  const verifyEmailOtp = async (data) => {
    const res = await verifyEmailOtpApi(data);
    if (!res.ok || !res.data?.accessToken) {
      return { success: false, error: res.data?.error || 'Verification failed. Please try again.', code: res.data?.code };
    }
    setTokens(res.data.accessToken, res.data.refreshToken);
    const u = res.data.user;
    const user = {
  ...
  const signup = async (name, email, password, mobile = null, product = null) => {
    try {
      const res = await apiFetch('/auth/register', {
        method: 'POST',
        body: { name, email, password, mobile, product },
        auth: false,
      });
      if (res.ok && res.data?.accessToken) {
        setTokens(res.data.accessToken, res.data.refreshToken);
  ...
  const uploadProfilePhoto = async (file) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      return { success: false, error: 'File size exceeds 5 MB. Please choose a smaller image.' };
    }
    ...
    return new Promise((resolve) => {
      ...
      reader.onload = async () => {
        try {
          ...
          const res = await uploadAvatarApi(base64, file.type);
          if (res.ok && res.data?.avatarUrl) {
            const formattedUrl = formatAvatarUrl(res.data.avatarUrl, getAccessToken());
            setAuthState((prev) => ({
              ...prev,
              user: prev.user ? { ...prev.user, avatarUrl: formattedUrl } : null,
            }));
            resolve({ success: true, avatarUrl: formattedUrl, user: res.data.user });
          } else {
            resolve({ success: false, error: res.data?.error || 'Failed to upload profile photo.' });
          }
        } catch (err) {
          resolve({ success: false, error: err.message || 'Profile photo upload failed.' });
        }
      };
  ...
  const removeProfilePhoto = async () => {
    try {
      ...
      if (res.ok) {
        setAuthState((prev) => ({
          ...prev,
          user: prev.user ? { ...prev.user, avatarUrl: null } : null,
        }));
        return { success: true };
      }
      return { success: false, error: res.data?.error || 'Failed to remove profile photo.' };
    } catch (err) {
      return { success: false, error: err.message || 'Profile photo removal failed.' };
    }
  };
  ...
  const hasProductAccess = useCallback((product) => {
    ...
    const canonical = raw === 'customer-portal' || raw === 'portal'
      ? 'customer-portal'
      : raw === 'sales-crm' || raw === 'crm'
      ? 'crm'
