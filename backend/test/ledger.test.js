...
async function runLedgerVerification() {
  ...
  await pool.query(`INSERT INTO users (id, email, password_hash, role, full_name) VALUES ($1, $2, 'hash', 'customer', 'Ledger User A') ON CONFLICT (id) DO NOTHING`, [testUserA, `ledger-a-${Date.now()}@test.com`]);
  await pool.query(`INSERT INTO users (id, email, password_hash, role, full_name) VALUES ($1, $2, 'hash', 'customer', 'Ledger User B') ON CONFLICT (id) DO NOTHING`, [testUserB, `ledger-b-${Date.now()}@test.com`]);
