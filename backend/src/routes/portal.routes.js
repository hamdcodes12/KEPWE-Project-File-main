...
const router = Router();
router.use(['/portal', '/customer-portal'], requireProductAccess('customer-portal'));
 ...
 */
router.get(['/portal/profile', '/customer-portal/profile'], requireAuth, async (req, res, next) => {
  try {
 ...
 */
router.get(['/portal/snapshot', '/customer-portal/snapshot'], requireAuth, async (req, res, next) => {
  try {
    const company = await getPrimaryCompanyForUser(req.userId);
    if (!company) {
 ...
 */
router.get(['/portal/tasks', '/customer-portal/tasks'], requireAuth, async (req, res, next) => {
  try {
    const company = await getPrimaryCompanyForUser(req.userId);
 ...
 */
router.get(['/portal/documents', '/customer-portal/documents'], requireAuth, async (req, res, next) => {
  try {
    const company = await getPrimaryCompanyForUser(req.userId);
 ...
 */
router.get(['/portal/documents/:documentId/download', '/customer-portal/documents/:documentId/download'], requireAuth, async (req, res, next) => {
  try {
    const result = await pool.query(
 ...
 */
router.post(['/portal/documents', '/customer-portal/documents'], requireAuth, validateBody(uploadDocSchema), async (req, res, next) => {
  try {
    const company = await getPrimaryCompanyForUser(req.userId);
    if (!company) {
