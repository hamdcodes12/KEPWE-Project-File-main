...
async function sendViaResend({ email, otp }) {
  ...
  if (!apiKey) {
    const error = new Error('Email delivery is not configured. Please contact support.');
  ...
  try {
    ...
    const error = new Error('Email delivery service is temporarily unavailable. Please try again.');
...
export async function requestEmailOtp({ email, purpose, payload = {} }) {
  ...
  if ((recent.rows[0]?.count ?? 0) >= MAX_REQUESTS_PER_HOUR) {
